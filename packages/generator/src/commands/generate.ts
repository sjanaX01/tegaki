...
export interface ExtractBundleInput {
   ...
   * is stored as `fallbackFamily` so the renderer can fall back to it for
...
export function extractTegakiBundle(input: ExtractBundleInput): TegakiBundleOutput {
  const { fontBuffer, fontFileName, chars: charsStr, options, onProgress, extraFontBuffers, subset = true } = input;
  ...
  // collide with a user-loaded full font. The original name is stored as
  // fallbackFamily so the renderer can use it as a CSS fallback.
  const bundleFamily = subset ? `${fontInfo.family} Tegaki ${charsHash(charsStr)}` : fontInfo.family;
  const fallbackFamily = subset ? fontInfo.family : undefined;
...
function generateGlyphsModule(
  ...
  const fallbackLine = fallbackFamily ? `\n  fallbackFamily: '${esc(fallbackFamily)}',` : '';
...
const bundle = {
  family: '${esc(fontFamily)}',${fallbackLine}
  lineCap: '${lineCap}',
  fontUrl,
  fontFaceCSS: \`@font-face { font-family: '${esc(fontFamily)}'; src: url(\${fontUrl}); }\`,
